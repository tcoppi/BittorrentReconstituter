...
distributed through DHT, which could have a big impact on the practical
usefulness of our program, since right now the \texttt{.torrent} file has to be
capture through some external means if a full extraction and verification is
needed.  As of now, we're assuming that the \texttt{.torrent} file has to
somehow magically be captured separately from the BitTorrent traffic.  If we
can incorporate DHT technology into our program, we can eliminate this
constraint.
